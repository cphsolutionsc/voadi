...
export function InstallPrompt() {
  ...
  return (
    <>
      <div className="fixed left-0 right-0 top-0 z-50 border-b border-[#E5E7EB] bg-white/95 backdrop-blur-2xl" style={{ WebkitBackdropFilter: 'blur(24px)' }}>
        <div className="mx-auto max-w-lg px-4 py-3">
          <div className="flex items-center gap-3">
            {/* Logo mark */}
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-[#9CA3AF] bg-[#FFFFFF]">
              <HarpV size={26} />
            </div>
            ...
            <div className="flex shrink-0 items-center gap-2">
              {platform === 'android' && (
                <button
                  ...
                  className="rounded-full bg-[#D97706] px-4 py-1.5 text-xs font-bold text-[#111827] transition-opacity hover:opacity-90"
                >
                  Install
                </button>
              )}
              ...
              <button
                onClick={dismiss}
                className="flex h-7 w-7 items-center justify-center rounded-full border border-[#E5E7EB] text-[#9CA3AF] transition-colors hover:text-[#4B5563]"
                aria-label="Dismiss"
              >
                <X size={13} />
              </button>
...
function Step({ n, icon, text }: { n: number; icon?: React.ReactNode; text: string }) {
  return (
    <div className="flex items-center gap-2.5 text-[11px] text-[#6B7280]">
      <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-[#E5E7EB] text-[9px] font-bold text-[#D97706]">
        {n}
      </span>
